...
def test_shuffle_series():
    ...
    assert np.mean(DF['S1']) == pytest.approx(np.mean(DF_shuffled['S1']), abs=1e-7)
    #assert_almost_equal(np.mean(DF['S1']), np.mean(DF_shuffled['S1']))
    assert np.mean(DF['S2']) == pytest.approx(np.mean(DF_shuffled['S2']), abs=1e-7)
    ...
    # Ensure rows are shuffled but not lost
    assert np.mean(DF['S1']) == pytest.approx(np.mean(S1_shuffled['S1']), abs=1e-7)
    assert np.mean(DF['S2']) == pytest.approx(np.mean(S1_shuffled['S2']), abs=1e-7)
...
def test_get_pdf():
    ...
    for a in alphas:
        for mean in means:
            ...
            assert pdf_hist.sum() == pytest.approx(1., abs=1e-8)
            assert pdf_kde.sum() == pytest.approx(1., abs=1e-8)
    ...
    assert np.average(cdfs_kde) == pytest.approx(0.5, abs=1e-1)
    assert np.average(cdfs_hist) == pytest.approx(0.5, abs=1e-1)
...
def test_joint_entropy():
    ...
    assert get_entropy(data,gridpoints=gridpoints) == pytest.approx(entropy(pdf,base=2), abs=1e-5)
...
def test_joint_entropyND():
    ...
    ## The estimated entropy should correspond to scipy's value (to 5 d.p.) 
    assert get_entropy(data,gridpoints=gridpoints) == pytest.approx(entropy(pdf.flatten(),base=2), abs=1e-5)
